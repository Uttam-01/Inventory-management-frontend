...
export default function () {
  ...
  const { data : Vendors, isLoading : isVendorLoading , error : vendorError } = useVendors();
  useEffect(() => {
    ...
    >
      ...
      <div className="mt-[25px]">
        <div className="flex  flex-wrap gap-x-6 gap-y-8 mt-[10px]">
                ...
                <div className="w-full">
                  ...
                  <ul className="  w-full h-[300px] overflow-y-auto bg-white ">
                    {filteredComponents.length === 0 && (
                      <li className="p-2 text-gray-400">No components found</li>
                    )}
                    {filteredComponents.map((component,i) => (
                      <li
                        ...
                        onClick={() => {setSelected(component); setIsOpen(false)}}
                      >
                        {component.name}
                      </li>
                    ))}
                ...
                <div className="w-full">
                  ...
                  <div className="  w-full h-[300px] overflow-y-auto bg-white ">
                    {filteredVendors.length === 0 && (
                      <div className="p-2 text-gray-400">No components found</div>
                    )}
                    {filteredVendors.map((k,i) => ( 
                      ...
                      <div
                        ...
                        onClick={() => {setSelectedVendor(k); setVendorOpen(false)}}
                      >
                        {k.name}
                      </div>
                    ))}
          ...
          <InputBox label="Per Unit Price" placeholder="Per Unit Price" />
          <InputBox label="Delivery Time(In Days" placeholder="Delivery Time" />
        </div>
      </div>
      ...
      <div className="w-full flex h-[42px] items-center justify-end gap-4 mt-[25px]">
        ...
        <div className=" h-[42px]  rounded-[8px] w-[200px]  text-[#FFFFFF] bg-[#0F4C81] text-[16px] text-emoji font-normal flex items-center justify-center">
