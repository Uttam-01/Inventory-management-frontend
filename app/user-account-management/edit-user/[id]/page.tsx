...
export default function () {
  ...
  useEffect(() => {
    if (userInfo?.roles && Array.isArray(userInfo.roles)) {
      const roleNames = userInfo.roles.map((role: any) => role.roleName);
  ...
  function handleFormSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const pathParts = pathname.split("/").filter(Boolean);
      const userId = pathParts[pathParts.length - 1];
    updateUserMutation.mutate({reqData : result.data , id : Number(userId)});
  }
      ...
      >
        ...
        <form onSubmit={handleFormSubmit}>
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-8 mt-[10px]">
              ...
              <div className="relative flex flex-col">
                ...
                <select
                  ...
                  name="accountStatus"
                  id=""
                  defaultValue={userInfo?.accountStatus}
                >
          ...
          <div className="w-full flex h-[42px] items-center justify-between gap-4 mt-[25px]">
            ...
            {updateUserMutation.isError && <div className="text-red-500 w-full">Error adding User.</div>}
